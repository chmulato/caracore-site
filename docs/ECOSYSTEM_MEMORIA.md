...
3. **Produtos principais (só estes três):** PDV (Java Free `v3.2.6-free` + candidato `v4.0.0-rc2` / **frente GA 08/11/2026** se T032 + Rust `v0.1.4`) · CSO (Frotas **no ar e em andamento**; freeze M1 08/11/2026; loja `cso-transp`; Transportes **08/11/2028**; ≠ GPS) · Hub (encomendas; GA Windows 06/04/2027). **Brincos:** Ink, OIDC, Seed, Circuito, Área 51, RU, Helianto, MKT, Minerador — existem; em conflito de cota ou headline, **cedem**.
...
6. **Hub oficina (GA Windows 06/04/2027):** `caracore-hub/docs/contexto-rapido.md` · `.cursor/rules/project-memory.mdc` · backend Java 25 validado (`core` 606 testes, `persistence` 541, `api` verde, `web` 274, WAR gerado). Em 22/09/2026: 11 migrações Flyway validadas no SQLite; casts Postgres (`jsonb`, `now`, `uuid`) saneados em `VolumeDAO`/`IntegracaoEventoDAO`; RBAC limpo (`ADMIN/SUPERVISOR/OPERADOR`); Spring expurgado do Electron. **Web 2.1 concluída na oficina; WAR não é release pública.** Trabalho aberto = empacotamento do instalador Windows/Electron com Tomcat embutido e SQLite WAL. Manual: wiki.caracore.com.br/hub/

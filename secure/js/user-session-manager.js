...
(function() {
    ...
    class UserSessionManager {
         ...
         * @returns {Object} - {isDifferentUser: boolean, isDifferentProvider: boolean, reason: string}
         */
        detectUserChange(newUserEmail, newUserProvider = null) {
            if (!newUserEmail) {
                return { isDifferentUser: false, isDifferentProvider: false, reason: 'no_email' };
            }
            ...
            const normalizedNewProvider = this.normalizeProvider(newUserProvider);
            const normalizedStoredProvider = this.normalizeProvider(stored.provider);
            
            // Se não há usuário armazenado, não é um usuário diferente
            if (!storedEmail) {
                return { isDifferentUser: false, isDifferentProvider: false, reason: 'no_stored_user' };
            }
            ...
            // Se o provider mudou para o mesmo email, limpar dados do provider anterior
            if (normalizedNewProvider && normalizedStoredProvider && 
                normalizedNewProvider !== normalizedStoredProvider) {
                console.log(`🔄 Provider mudou para ${newEmail}: ${normalizedStoredProvider} -> ${normalizedNewProvider}`);
                return { 
                    isDifferentUser: false, 
                    isDifferentProvider: true, 
                    reason: 'different_provider_same_user' 
                };
            }
            
            return { isDifferentUser: false, isDifferentProvider: false, reason: 'same_user_same_provider' };
        }
        
        /**
         * Normalizar nome do provider (entra, azure, microsoft -> microsoft)
         * @param {string} provider - Nome do provider
         * @returns {string} - Provider normalizado
         */
        normalizeProvider(provider) {
            if (!provider) return null;
            const normalized = provider.toLowerCase().trim();
            // Normalizar variações do Microsoft
            if (normalized === 'entra' || normalized === 'azure' || normalized === 'microsoft') {
                return 'microsoft';
            }
            // Normalizar Google
        ...
        /**
         * Detectar se um novo usuário está fazendo login (método legado para compatibilidade)
         * @param {string} newUserEmail - Email do novo usuário
         * @param {string} newUserProvider - Provider do novo usuário
         * @returns {boolean} - true se é um usuário diferente
         */
        isDifferentUser(newUserEmail, newUserProvider = null) {
            const change = this.detectUserChange(newUserEmail, newUserProvider);
            return change.isDifferentUser || change.isDifferentProvider;
        }

...
6. **Hub oficina (GA Windows 06/04/2027):** `caracore-hub/docs/contexto-rapido.md` · `.cursor/rules/project-memory.mdc` · backend Java 25 validado (`core` 606 testes, `persistence` 541, `api` verde, `web` 274, WAR gerado). Em 22/09/2026: 11 migrações Flyway validadas no SQLite; casts Postgres (`jsonb`, `now`, `uuid`) saneados em `VolumeDAO`/`IntegracaoEventoDAO`; RBAC limpo (`ADMIN/SUPERVISOR/OPERADOR`); Spring expurgado do Electron. Trabalho aberto = instalador Windows/Electron. Manual: wiki.caracore.com.br/hub/
